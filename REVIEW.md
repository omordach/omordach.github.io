...
- **Linting Fixes for Fast Refresh:**
  ...
  - _Fix:_ Since this is intentional for UI libraries (like `class-variance-authority`), either add `// eslint-disable-next-line react-refresh/only-export-components` to the bottom of the UI files, OR update `eslint.config.js` to ignore `src/components/ui/**` for that specific rule since these are shared structural primitives.
